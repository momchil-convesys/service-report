...
import { chartOptions_PV, updateChartData_PV } from './chart-manipulation-pv';
import { updatePlotBandsForCurrentInterval } from './chart-plotbands-current-interval';

@Component({
  selector: 'app-power-schedule-tracking-chart-pv',
  templateUrl: './power-schedule-tracking-chart-pv.component.html',
  styleUrls: ['./power-schedule-tracking-chart-pv.component.less'],
  imports: [],
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class PowerScheduleTrackingChartPVComponent
  ...
  implements OnChanges, OnDestroy
{
  ...
  private _updateChartDataFromRequest(): void {
    ...
    chartData.exportFileName = `power-schedule-tracking-pv-${chartData.plantId}-${DataAdapter.modelToDtoTimestamp(chartData.from)}-${DataAdapter.modelToDtoTimestamp(chartData.to)}`;
  ...
  override updateChartData(chart: Highcharts.Chart, data: PowerScheduleTracking | undefined): void {
    ...
    updateChartData_PV(chart, data, this.context, this._clockService);
