...
import { chartOptions_BESS, updateChartData_BESS } from './chart-manipulation-bess';
import { updatePlotBandsForCurrentInterval } from './chart-plotbands-current-interval';

@Component({
  selector: 'app-power-schedule-tracking-chart-bess',
  templateUrl: './power-schedule-tracking-chart-bess.component.html',
  styleUrls: ['./power-schedule-tracking-chart-bess.component.less'],
  imports: [],
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class PowerScheduleTrackingChartBESSComponent
  ...
  implements OnChanges, OnDestroy
{
  ...
  private _updateChartDataFromRequest(): void {
    ...
    chartData.exportFileName = `power-schedule-tracking-bess-${chartData.plantId}-${DataAdapter.modelToDtoTimestamp(chartData.from)}-${DataAdapter.modelToDtoTimestamp(chartData.to)}`;
  ...
  override updateChartData(chart: Highcharts.Chart, data: PowerScheduleTracking | undefined): void {
    this._chart?.xAxis[0].update(
      {
        labels: {
          enabled: !this.optimizeForDashboard,
        },
        tickLength: this.optimizeForDashboard ? 0 : undefined,
      },
      false,
    );

    updateChartData_BESS(chart, data, this.context, this._clockService);
