...
// import { MaterialsPickerService } from '../../../../materials/materials-picker.service';
import { SchematicElement } from '../../../../models/_service-reports';
...
export class MaterialsSchemaViewComponent implements AfterViewInit, OnDestroy {
  ...
  @Output() elementSelectedToggle = new EventEmitter<boolean>();
  rawSvg!: SafeHtml;
  ...
  constructor(
    ...
    this.materialsPickerService.elementSelected$
      ...
      .subscribe((el) => {
        ...
        this.elementSelectedToggle.emit(true);
      });
    ...
    this.materialsPickerService.elementDeselected$
      ...
      .subscribe((el) => {
        ...
        this.elementSelectedToggle.emit(false);
      });
  ...
  ngAfterViewInit() {
    ...
    // const schemaUrl = 'http://localhost:4444/res/schemas/SOLO500-Decorated-cf2f255.svg';

    //const schemaUrl = apiBaseUrl + this.schema.files[0].substring(1);
    const apiBaseUrl = window.apiBaseUrl;

    if (this.schema) {
      this.http
        .get(`${apiBaseUrl}/service-reports/schemas/download/${this.schema.id}`, {
          headers,
          responseType: 'text',
        })
        .pipe(
  ...
  private clickHandler(event: any) {
    ...
    if (target.getAttribute('data-selected') === '0') {
      ...
      const group = this.materialsPickerService.getSelectedFG(element);
      if (this.materialsPickerService.isOkdeselect(group)) {
  ...
  private setSvg(svg: string) {
    this.rawSvg = this.sanitizer.bypassSecurityTrustHtml(svg);
